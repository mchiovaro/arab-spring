...
instances and constrain them in others” [@kitschelt1986political]. Emergent strategies and tactics in contentious politics are a function of
...
only events that were directed at (i.e., `target`) Syria ($n = INSERT NUMBER HERE$). To
...
by (i.e., `source`) Syria ($n = INSERT NUMBER HERE$).
...
Equation 1 identifies the inverse frequency weighting of each overlapping word ti between two tweets d_1 and d_2, relative to the word’s frequency in the corpus D, for all overlapping words t1...k between the two tweets.
...
Equation 2 then calculates the inverse frequency of each word ti  in tweets d_1 and d_2, relative to the word’s frequency in the corpus D, for all words t1...n that appear in either tweet.
...
raw_cohesion_data <- read.csv("./data/raw/syria_cohesion.csv")
...
raw_cohesion_data %>%
  group_by(date) %>%
  summarise(
    MeanCohesion = mean(Cohesion_avg),
    sd = sd(Cohesion_avg),
    var = sd^2,
    n = n(),
    se = sd / n) %>%
...
For both the target-only (n = `r num_target`) and the target and source data (n = `r num_source_target`) described above, time
...
CRQ analyses. Given that a permutation test is similar to bootstrapping but does not include replacement, all real recurrent points were included in each CRQA analysis, leaving RR constant across all permutations. Thus, RR is not included in the inferential stests.
...
found a statistically significant difference from chance for maxL. All other metrics did not reach significance (Figure \@ref(fig:plot-rp-targ); \autoref{table-1}).
...
A permutation test for the CRQA of social cohesion and count of all events found
a statistical difference from chance for maxL and LAM. All other metrics
were non-significant (Figure \@ref(fig:plot-rp-source-targ);
\autoref{table-2}).

A permutation test for the CRQA of social cohesion and count of positive events
found a statistical difference from chance for DET, NRLINE, maxL, L, ENTR, and LAM (Figure
\@ref(fig:plot-rp-source-targ); \autoref{table-2}).

A permutation test for the CRQA of social cohesion and count of negative events
found a statistical difference from chance for rENTR. All other
...
above-chance RR from the 2nd--12th and 44th--50th window. DET was largely above chance
from the 47th to the 52nd window, with some exceptions (Figure
...
above-chance RR until the 18th window, with an exception for window 17. A peak also appeared for the 40th--42nd windows. DET was intermittently at or above chance for the 8th--14th and 54th--62nd windows (Figure \@ref(fig:plot-wcrqa-targ-all)).

For social cohesion and count of negative events, RR stayed above or almost
above chance from the 4th--7th and 46th--49th windows and was below chance for extended period from the 56th--62nd window. DET bottomed out at zero for the 15th--23rd and 34th--39th windows. The metric was above or nearly above chance only toward the end of the time series, in the 47th--54th and 62nd windows (Figure \@ref(fig:plot-wcrqa-targ-all)).
...
For social cohesion and count of all events, there was a consistent period of
above- or nearly above-chance DET from the 2nd--18th and 44th--52nd windows. DET
also showed above- or nearly above-chance values in windows 7--11 and 59--62 (Figure
\@ref(fig:plot-wcrqa-source-targ-all)).

For social cohesion and count of positive events, there was a downward trend in
RR as the window moved, with above-chance values in the first 23 windows
and the 57th--62nd windows. DET stayed largely within chance, with the exception
of windows 1 and 4 (Figure \@ref(fig:plot-wcrqa-source-targ-all)).

For social cohesion and negative events, RR was above chance for the 4th--6th and 46th--49th windows and below chance for the 56th--62nd windows. DET showed an upward trend as the windows moved. It bottomed out to zero for the 15th--23rd and 34th--39th windows, and was only above change for the 47th--54th and 62nd windows. (Figure
\@ref(fig:plot-wcrqa-source-targ-all)).
