...
tweets. The servers were run on a stable (LTS) version of Ubuntu Linus (v.
...
for a single group cohesion value. The cohesion value for each day was
quantified as the average cohesion values for all groups in that day. Each
pair’s cohesion value was calculated in three parts:
...
tweets. All hashtags and URLs were included in the analysis. The raw social
cohesion time series can be seen in Figure \@ref(fig:raw-cohesion-ts).
...
[Figures \@ref(fig:raw-event-ts-target) and
...
not include replacement, all real recurrent points were included in each CRQA
analysis, leaving RR constant across all permutations. Thus, RR is not included
in the inferential tests.
...
structure and thus a more meaningful interpretation of DET. A window size
smaller than 14 would limit the possibilities for shared trajectories and make
determinism uninterpretable, and window sizes larger than 14 would increasingly
overshadow the fine-grain dynamics that we want to investigate.
...
did not reach significance (Figure \@ref(fig:plot-rp-targ); \autoref{table-1}).

A permutation test for the CRQA of social cohesion and count of positive events
found a statistical difference from chance for DET, NRLINE, maxL, LAM, and TT.
All other metrics did not reach significance (Figure \@ref(fig:plot-rp-targ);
\autoref{table-1}).
...
maxL. All other metrics did not reach significance (Figure
...
were non-significant (Figure \@ref(fig:plot-rp-source-targ); \autoref{table-2}).

A permutation test for the CRQA of social cohesion and count of positive events
found a statistical difference from chance for DET, L, ENTR, rENTR, and LAM
(Figure \@ref(fig:plot-rp-source-targ); \autoref{table-2}).

A permutation test for the CRQA of social cohesion and count of negative events
found no statistical difference from chance for and metrics (Figure
...
above chance from the 47th to the 54th window, with some exceptions (Figure
...
chance for the 8th--14th and 54th--62nd windows (Figure
\@ref(fig:plot-wcrqa-targ-all)).
...
end of the time series, in the 47th--54th and 62nd windows (Figure
\@ref(fig:plot-wcrqa-targ-all)).
...
above- or nearly above-chance values in windows 7--11 and 59--62 (Figure
...
bottoming-out to a value of 0 (Figure \@ref(fig:plot-wcrqa-source-targ-all)).
...
52nd--62nd windows. (Figure \@ref(fig:plot-wcrqa-source-targ-all)).
