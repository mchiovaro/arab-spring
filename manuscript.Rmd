...
example, the pervasive “I am a man” posters during the 1968 sanitation workers'
...
@goldstein1992conflict]. These event codes include a range of behaviors, from
...
The corpus of tweets used included `r tweet_stats$total_tweets` Syrian tweets
...
@robertson2004understanding]. However, this method focuses more squarely on terms (i.e., agnostic to the number of documents in the corpus), aggregates at a higher semantic level (i.e., summing across words), and accounts for linguistic novelty relative to the pair and the corpus (i.e., ratio of Eq. 1 to Eq. 2). The social cohesion calculation was performed on the tweet corpus containing both English and Arabic tweets. All hashtags and URLs were included in the analysis. The raw social cohesion time series can be seen in Figure \@ref(fig:raw-cohesion-ts).
...
@steinert2017spontaneous. By introducing the CRQA and WCRQA methods, we
...
chance. This could be due to a conflation of the different kinds of events. However, when we specifically target positive events, the coupling with social cohesion is strong, running counter to our second hypothesis. In addition to the significant DET and NRLINE metrics seen in the all events analysis, maxL and TT are significant for positive events, suggesting a more stable relationship. This may be a reflection of citizens' reaction to events that are positively influential to their cause. An act of concession by the government or aid from another country may have led to an increase in morale or feelings of hope, in turn leading to an uproar on Twitter in an attempt to boost morale. By contrast, negative event frequency showed weak coupling, counter to our third hypothesis. Although the significant result for maxL suggests stability, the lack of significant results for other metrics indicate that the maxL metric may point toward a particular period where coupling was strong, but that negative events were less coupled for the other points in time. Perhaps during times of intense strife, individuals in Syria focused on real-world action, turning to Twitter in the following days to report post-hoc what had happened. Further text analysis should be conducted to decipher the underlying tone of the tweets. 
...
within the country were already captured by the target-only filtering. Taking outwardly directed events into account dramatically changed the coupling between all events and social cohesion. There is no longer a significant number of shared trajectories or line lengths, indicating that the cohesion on Twitter is not tied to the frequency of overall daily events. Since source events are at the root of this difference, it is plausible that Syrians were focused on the target events as they are immediately impacting them, and the addition of the source events now overshadows the relationship between target events and social cohesion online. In contrast, there was a slight increase in coupling strength between positive events and social cohesion that once again can only be due to the externally directed events. These two time series showed greater stability in their coupling (reflected in L). But the significant ENTR metric suggests that there is more variability in the coupling, and thus we do not feel that the source events impacted the positive event coupling notably, but rather broke up the trajectories formed by the target events. Although the coupling between social cohesion with negative events were largely unchanged, there was a slight decrease in coupling strength (reflected in maxL) and increase in complexity of shared trajectories (reflected in rENTR). Similarly to that of positive events, this change may not be reflective of source events themselves, but rather a disruption of the relationship with target events.
