...
Given the inherent issues found in event data sets [@wang2016growing], we conducted the same analyses on the Cline Center Historical Pheonix Event Data [@illinoisdatabankIDB-2796521]. After filtering the data and creating the count variables for all, positive, and negative events each day, there was not enough variability to transform the data into deciles for the target only Pheonix data. As a results, sextiles were used for these analyses. For comparison, the filtered ICEWS data was also transformed into sextiles. 

## CRQA for Pheonix data in sextiles
```{r secondary-crqa-phoenix, , echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, results='hide'}

# run crqa on sextiled pheonix data
source('./scripts/secondary-analyses/pheonix/02_crqa.R')

# read in metric names
pheonix_metrics <- t(read.csv("./results/secondary/pheonix/crqa/target/crqa_results-all_events.csv", header = FALSE)[-2,-1])

# read in metrics from crq analyses (target filtered)
pheonix_metrics_all_targ <- t(read.csv("./results/secondary/pheonix/crqa/target/crqa_results-all_events.csv", header = FALSE)[-1,-1])
pheonix_metrics_pos_targ <- t(read.csv("./results/secondary/pheonix/crqa/target/crqa_results-pos_events.csv", header = FALSE)[-1,-1])
pheonix_metrics_neg_targ <- t(read.csv("./results/secondary/pheonix/crqa/target/crqa_results-neg_events.csv", header = FALSE)[-1,-1])

# read in signficiance from permutation tests (target filtered)
pheonix_sig_all_targ <- t(read.csv("./results/secondary/pheonix/crqa/target/metric_signif_all.csv", header = FALSE)[-1,-1])
pheonix_sig_pos_targ <- t(read.csv("./results/secondary/pheonix/crqa/target/metric_signif_pos.csv", header = FALSE)[-1,-1])
pheonix_sig_neg_targ <- t(read.csv("./results/secondary/pheonix/crqa/target/metric_signif_neg.csv", header = FALSE)[-1,-1])

# read in metrics from crq analyses (source and target filtered)
pheonix_metrics_all_source_targ <- t(read.csv("./results/secondary/pheonix/crqa/source_target/crqa_results-all_events.csv", header = FALSE)[-1,-1])
pheonix_metrics_pos_source_targ <- t(read.csv("./results/secondary/pheonix/crqa/source_target/crqa_results-pos_events.csv", header = FALSE)[-1,-1])
pheonix_metrics_neg_source_targ <- t(read.csv("./results/secondary/pheonix/crqa/source_target/crqa_results-neg_events.csv", header = FALSE)[-1,-1])

# read in signficiance from permutation tests (source and target filtered)
pheonix_sig_all_source_targ <- t(read.csv("./results/secondary/pheonix/crqa/source_target/metric_signif_all.csv", header = FALSE)[-1,-1])
pheonix_sig_pos_source_targ <- t(read.csv("./results/secondary/pheonix/crqa/source_target/metric_signif_pos.csv", header = FALSE)[-1,-1])
pheonix_sig_neg_source_targ <- t(read.csv("./results/secondary/pheonix/crqa/source_target/metric_signif_neg.csv", header = FALSE)[-1,-1])

```

```{r secondary-crqa-table-pheonix-target, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}

# bind metrics and significance results
pheonix_target_results <- as.data.frame(cbind(pheonix_metrics,
                                      pheonix_metrics_all_targ, 
                                      pheonix_sig_all_targ, 
                                      pheonix_metrics_pos_targ, 
                                      pheonix_sig_pos_targ, 
                                      pheonix_metrics_neg_targ, 
                                      pheonix_sig_neg_targ), 
                                      stringsAsFactors = FALSE)

# covert to numerics for rounding
numerics <- c(2:7)
pheonix_target_results[,numerics] <- sapply(pheonix_target_results[,numerics],as.numeric)

# add significance stars
stars_all <- as.data.frame(stars.pval(pheonix_target_results[,3]))
stars_pos <- as.data.frame(stars.pval(pheonix_target_results[,5]))
stars_neg <- as.data.frame(stars.pval(pheonix_target_results[,7]))

pheonix_target_results <- cbind(pheonix_target_results, stars_all, stars_pos, stars_neg)
pheonix_target_results <- pheonix_target_results[,c(1,2,3,8,4,5,9,6,7,10)]

# give intuitive names
colnames(pheonix_target_results) <- c("metric", "all events", "p", "sig.", "positive events", "p", "sig.", "negative events", "p", "sig.")

# exclude row names
rownames(pheonix_target_results) <- NULL
...
# build table
pander(pheonix_target_results, caption = "CRQA results for target only Pheonix data.")

```

```{r primary-table-pheonix-source-target, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}

# bind metrics and significance results
pheonix_source_target_results <- as.data.frame(cbind(pheonix_metrics,
                                      pheonix_metrics_all_source_targ, 
                                      pheonix_sig_all_source_targ, 
                                      pheonix_metrics_pos_source_targ, 
                                      pheonix_sig_pos_source_targ, 
                                      pheonix_metrics_neg_source_targ, 
                                      pheonix_sig_neg_source_targ), 
                                      stringsAsFactors = FALSE)

# covert to numerics for rounding
numerics <- c(2:7)
pheonix_source_target_results[,numerics] <- sapply(pheonix_source_target_results[,numerics],as.numeric)

# add significance stars
stars_all <- as.data.frame(stars.pval(pheonix_source_target_results[,3]))
stars_pos <- as.data.frame(stars.pval(pheonix_source_target_results[,5]))
stars_neg <- as.data.frame(stars.pval(pheonix_source_target_results[,7]))

pheonix_source_target_results <- cbind(pheonix_source_target_results, stars_all, stars_pos, stars_neg)
pheonix_source_target_results <- pheonix_source_target_results[,c(1,2,3,8,4,5,9,6,7,10)]

# give intuitive names
colnames(pheonix_source_target_results) <- c("metric", "all events", "p", "sig.", "positive events", "p", "sig.", "negative events", "p", "sig.")

# exclude row names
rownames(pheonix_source_target_results) <- NULL
...
# build table
pander(pheonix_source_target_results, caption = "CRQA results for source and target Pheonix data.")

```
CRQ analyses for sextiled Pheonix event data found increased levels of coupling between positive events and social cohesion, irrespective of target or source and target filtering. Negative events were not coupled with social cohesion irrepective of filtering. 
...
```{r primary-table-icews-source-target, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
...
The abscence of coupling between negative events and social cohesion in the Pheonix CRQ analyses speaks to the unreliability of event data, and that nonlinear analyses in general are only as valuable as the quality of the data being used. 

## Windowed CRQA for Pheonix data in sextiles
```{r secondary-windowed-crqa-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, results='hide'}

# run windowed crqa on sextiled pheonix data
...
```{r plot-RR-targ-all-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of RR for social cohesion and count of all events from target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/target/windowed_all_RR.png")

```

```{r plot-DET-targ-all-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of DET for social cohesion and count of all events from target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/target/windowed_all_DET.png")

```

```{r plot-RR-targ-pos-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of RR for social cohesion and count of positive events from target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/target/windowed_pos_RR.png")

```

```{r plot-DET-targ-pos-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of DET for social cohesion and count of positive events from target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/target/windowed_pos_DET.png")

```

```{r plot-RR-targ-neg-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of RR for social cohesion and count of negative events from target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/target/windowed_neg_RR.png")

```

```{r plot-DET-targ-neg-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of DET for social cohesion and count of negative events from target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/target/windowed_neg_DET.png")

```

### Syria as Source and Target

```{r plot-RR-source-targ-all-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of RR for social cohesion and count of all events from source and target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/source_target/windowed_all_RR.png")

```

```{r plot-DET-source-targ-all-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of DET for social cohesion and count of all events from source and target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/source_target/windowed_all_DET.png")

```

```{r plot-RR-source-targ-pos-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of RR for social cohesion and count of positive events from source and target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/source_target/windowed_pos_RR.png")

```

```{r plot-DET-source-targ-pos-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of DET for social cohesion and count of positive events from source and target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/source_target/windowed_pos_DET.png")

```

```{r plot-RR-source-targ-neg-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of RR for social cohesion and count of negative events from source and target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/source_target/windowed_neg_RR.png")

```

```{r plot-DET-source-targ-neg-secondary-pheonix, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = 'Windowed CRQA visualization of DET for social cohesion and count of negative events from source and target filtered data.'}

# call image
knitr::include_graphics("./results/secondary/pheonix/windowed-crqa/source_target/windowed_neg_DET.png")
