...
abstract: |
  ...
  nuanced connection to positive events, only visible when examining
  finer-grained data. This suggests that virtual mobilization does reflect
  real-world movements, with the strongest connections to the frequency of all
  events and the frequency of negative events. We conclude with a discussion of
  implications and applications of nonlinear methods in political science
  research.
...
individual, event, or political injustice [@goldstone1980theories;
...
group ready and mobilized for action [@Hoffer2011true].
...
others” [@kitschelt1986political]. What emerges as strategies and tactics in
...
The raw social cohesion time series can be seen in Figure \@ref(fig:raw-cohesion-ts).
...
The raw event time series for target filtered data can be seen in Figure \@ref(fig:raw-event-ts-target). The The raw event time series for source and target filtered data can be seen in Figure \@ref(fig:raw-event-ts-source-target).
...
ggplot() +
  geom_line(event_df, mapping = aes(x= as.Date(Date), y=all_events_target, colour = 'all events')) +
  geom_line(event_df, mapping = aes(x= as.Date(Date), y=pos_events_target, colour = 'positive events')) +
  geom_line(event_df, mapping = aes(x= as.Date(Date), y=neg_events_target, colour = 'negative events')) +
...
# plot the time series
ggplot() +
  geom_line(event_df, mapping = aes(x= as.Date(Date), y=all_events_source_target, colour = 'all events')) +
  geom_line(event_df, mapping = aes(x= as.Date(Date), y=pos_events_source_target, colour = 'positive events')) +
  geom_line(event_df, mapping = aes(x= as.Date(Date), y=neg_events_source_target, colour = 'negative events')) +
  scale_color_manual(values = c("red", "orange", "blue")) +
  xlab("Date") +
  ylab("Number of events") +
  ylim(0, 360)
...
#### Metrics
For WCRQA analyses, we decided to focus on RR and DET, but WCRQA visualization can be done for any metric desired.
